...
export POSTGRES_PASSWORD="${POSTGRES_PASSWORD:-acceptance-platform-password}"
export POSTGRES_RUNTIME_USER="${POSTGRES_RUNTIME_USER:-runtime}"
export POSTGRES_RUNTIME_PASSWORD="${POSTGRES_RUNTIME_PASSWORD:-acceptance-runtime-password}"
...
printf '%s' "canary-production-model-key" >"${secret_dir}/model-api-key"
printf '%s' "acceptance-minio" >"${secret_dir}/minio-access-key"
printf '%s' "acceptance-minio-password" >"${secret_dir}/minio-secret-key"
...
if grep -Eiq 'canary-production-model-key|authorization:|postgres://[^[:space:]]*password|acceptance-platform-password|acceptance-runtime-password' \
