...
func TestPostgresDatabaseIdentity(t *testing.T) {
	cfg, err := pgxpool.ParseConfig("postgres://queue-role:top-secret@db.internal:5432/agents?search_path=runtime")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, forbidden := range []string{"queue-role", "top-secret", "db.internal", "agents", "runtime"} {
