...
func TestMemoryConfirmedReplayAndPayloadConflict(t *testing.T) {
	...
	req := testReserve("tenant-a", "operation-1", "top-secret=never-persist")
	...
	for _, secret := range []string{"top-secret=never-persist", "worker-capability-secret", "sensitive-result"} {
...
func TestErrorsNeverEchoCallerData(t *testing.T) {
	secret := "password=hunter2 and bearer-token"
