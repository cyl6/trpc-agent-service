...
func TestPublicErrorMapsStableSafeResponses(t *testing.T) {
	cases := []struct {
		...
		{errors.New("provider echoed secret sk-do-not-leak"), http.StatusBadGateway, "model_request_failed", "模型调用失败，请检查 API Key、模型 ID 和请求地址"},
	}
	for _, test := range cases {
		...
		if strings.Contains(message, "sk-do-not-leak") {
