...
func TestAddCustomModelRegistersTenant(t *testing.T) {
	...
	if strings.Contains(body, "sk-test-123") {
	...
	if strings.Contains(list, "sk-test-123") {
...
func TestAddCustomModelRejectsInvalidInput(t *testing.T) {
	cases := map[string]string{
		"missing model id": `{"base_url":"https://api.example.com/v1","api_key":"sk-x"}`,
		"missing api key":  `{"base_url":"https://api.example.com/v1","model_id":"m"}`,
		"missing base url": `{"model_id":"m","api_key":"sk-x"}`,
		"relative url":     `{"base_url":"api.example.com/v1","model_id":"m","api_key":"sk-x"}`,
		"url credentials":  `{"base_url":"https://user:pass@api.example.com/v1","model_id":"m","api_key":"sk-x"}`,
		"url query":        `{"base_url":"https://api.example.com/v1?key=value","model_id":"m","api_key":"sk-x"}`,
		"empty url query":  `{"base_url":"https://api.example.com/v1?","model_id":"m","api_key":"sk-x"}`,
		"url fragment":     `{"base_url":"https://api.example.com/v1#fragment","model_id":"m","api_key":"sk-x"}`,
		"empty fragment":   `{"base_url":"https://api.example.com/v1#","model_id":"m","api_key":"sk-x"}`,
		"full url no path": `{"base_url":"https://api.example.com/v1","full_url":true,"model_id":"m","api_key":"sk-x"}`,
		"full url no flag": `{"base_url":"https://api.example.com/v1/chat/completions","model_id":"m","api_key":"sk-x"}`,
		"bad api format":   `{"api_format":"anthropic","base_url":"https://api.example.com/v1","model_id":"m","api_key":"sk-x"}`,
		"long display":     `{"base_url":"https://api.example.com/v1","model_id":"m","api_key":"sk-x","display_name":"0123456789012345678901234567890123"}`,
		"model id junk":    `{"base_url":"https://api.example.com/v1","model_id":"///","api_key":"sk-x"}`,
		"invalid json":     `{"model_id":`,
		"multiple objects": `{"base_url":"https://api.example.com/v1","model_id":"m","api_key":"sk-x"} {}`,
	}
...
func TestAddCustomModelRejectsDuplicateTenant(t *testing.T) {
	server := customModelServer(t, "")
	payload := `{"base_url":"https://api.example.com/v1","model_id":"llama-3","api_key":"sk-x"}`
...
func TestReloadKeepsCustomTenants(t *testing.T) {
	...
	if res := postCustomModel(t, server, `{"base_url":"https://api.example.com/v1","model_id":"kimi","api_key":"sk-x"}`); res.Code != http.StatusCreated {
		t.Fatalf("create status=%d body=%s", res.Code, res.Body.String())
	}
...
func TestCustomModelChatCallsConfiguredOpenAIEndpoint(t *testing.T) {
	...
	case request := <-captured:
		...
		if request.Authorization != "Bearer sk-integration" {
